import AppLayout from '@/components/Layouts/AppLayout'
import Head from 'next/head'
import Link from 'next/link'
import { useEffect, useState, useRef } from 'react'
import axios from 'axios'
import { useAuth } from '@/hooks/auth'
import { useReactToPrint } from 'react-to-print'
import QRCode from 'qrcode.react'

const Dashboard = () => {
    const { user } = useAuth({ middleware: 'auth' })
    const userId = user?.id
    const [getqr, setQRCode] = useState([])
    const baseuri = process.env.NEXT_PUBLIC_BACKEND_URL
    useEffect(() => {
        const fetchGetQRByUser = async () => {
            try {
                const response = await axios.get(
                    `${baseuri}/api/getqr/${userId}`,
                )
                setQRCode(response.data.getqr)
            } catch (error) {
                console.error('Error fetching GetQR:', error)
            }
        }

        fetchGetQRByUser()
        console.log(userId)
    }, [userId])

    const componentRef = useRef()

    const handlePrint = useReactToPrint({
        content: () => componentRef.current,
    })

    // ====================delete qr function --==========

    const handleDelete = async id => {
        try {
            const response = await axios.delete(`${baseuri}/api/deleteqr/${id}`)
            if (response.data.message) {
                // The Qrgen was deleted successfully
                alert('Qrgen deleted successfully')
                // Optionally, you might want to refresh the qrData or update the UI
            } else {
                // Handle cases where the deletion failed
                alert('Qrgen deletion failed')
            }
        } catch (error) {
            console.error('Error deleting Qrgen:', error)
            // Handle error if needed
        }
    }
    return (
        <AppLayout>
            <Head>
                <title>Laravel - Dashboard</title>
            </Head>

            <section>
                <div className="container">
                    <div className="row">
                        <div className="col-md-4">
                            <div className="sidebar">
                                <div className="sidebar-wrapper">
                                    <div className="sidebar-menu">
                                        <h2>My vCards</h2>
                                        <ul className="sidebar-list">
                                            <li>
                                                <Link href="/dashboard">
                                                    <div>
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            width="24"
                                                            height="24"
                                                            viewBox="0 0 24 24"
                                                            fill="none">
                                                            <path
                                                                d="M0 0.75C0 0.551088 0.0790176 0.360322 0.21967 0.21967C0.360322 0.0790176 0.551088 0 0.75 0L8.25 0C8.44891 0 8.63968 0.0790176 8.78033 0.21967C8.92098 0.360322 9 0.551088 9 0.75V8.6655L15.2205 2.469C15.3611 2.3284 15.5519 2.24941 15.7507 2.24941C15.9496 2.24941 16.1404 2.3284 16.281 2.469L21.5835 7.773C21.6533 7.84267 21.7088 7.92543 21.7466 8.01655C21.7844 8.10767 21.8038 8.20535 21.8038 8.304C21.8038 8.40265 21.7844 8.50033 21.7466 8.59145C21.7088 8.68257 21.6533 8.76533 21.5835 8.835L15.3915 15H23.25C23.4489 15 23.6397 15.079 23.7803 15.2197C23.921 15.3603 24 15.5511 24 15.75V23.25C24 23.4489 23.921 23.6397 23.7803 23.7803C23.6397 23.921 23.4489 24 23.25 24H4.5C3.90898 24.0007 3.32365 23.8846 2.77765 23.6583C2.23166 23.432 1.73577 23.1001 1.3185 22.6815C0.489166 21.8549 0.0159661 20.7368 0 19.566M9 19.251L19.992 8.301L15.7485 4.059L9 10.782V19.2525V19.251ZM6.75 19.5C6.75 18.9033 6.51295 18.331 6.09099 17.909C5.66903 17.4871 5.09674 17.25 4.5 17.25C3.90326 17.25 3.33097 17.4871 2.90901 17.909C2.48705 18.331 2.25 18.9033 2.25 19.5C2.25 20.0967 2.48705 20.669 2.90901 21.091C3.33097 21.5129 3.90326 21.75 4.5 21.75C5.09674 21.75 5.66903 21.5129 6.09099 21.091C6.51295 20.669 6.75 20.0967 6.75 19.5ZM22.5 22.5V16.5H13.887L7.8645 22.5H22.5Z"
                                                                fill="#D9D9D9"
                                                            />
                                                        </svg>
                                                        <span> All vCards</span>
                                                    </div>
                                                </Link>
                                            </li>
                                            <li>
                                                <Link href="/">
                                                    <div>
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            width="24"
                                                            height="24"
                                                            viewBox="0 0 24 24"
                                                            fill="none">
                                                            <path
                                                                fillRule="evenodd"
                                                                clipRule="evenodd"
                                                                d="M14.0282 2.35253L15.4098 3.76867L17.387 3.74467C17.7648 3.74025 18.1397 3.81137 18.4896 3.95386C18.8394 4.09634 19.1573 4.30732 19.4245 4.57439C19.6918 4.84146 19.9029 5.15924 20.0456 5.50906C20.1882 5.85888 20.2596 6.23369 20.2553 6.61146L20.2313 8.59166L21.6475 9.9718C21.9178 10.2359 22.1326 10.5513 22.2793 10.8997C22.426 11.248 22.5015 11.6221 22.5015 12C22.5015 12.3779 22.426 12.752 22.2793 13.1003C22.1326 13.4487 21.9178 13.7641 21.6475 14.0282L20.2313 15.4098L20.2553 17.387C20.2598 17.7648 20.1886 18.1397 20.0461 18.4896C19.9037 18.8394 19.6927 19.1573 19.4256 19.4245C19.1585 19.6918 18.8408 19.9029 18.4909 20.0456C18.1411 20.1882 17.7663 20.2596 17.3885 20.2553L15.4083 20.2313L14.0282 21.6475C13.7641 21.9178 13.4487 22.1326 13.1003 22.2793C12.752 22.426 12.3779 22.5015 12 22.5015C11.6221 22.5015 11.248 22.426 10.8997 22.2793C10.5513 22.1326 10.2359 21.9178 9.9718 21.6475L8.59016 20.2313L6.61296 20.2553C6.23519 20.2598 5.86035 20.1886 5.51045 20.0461C5.16056 19.9037 4.84266 19.6927 4.57545 19.4256C4.30824 19.1585 4.0971 18.8408 3.95443 18.4909C3.81176 18.1411 3.74045 17.7663 3.74467 17.3885L3.76867 15.4083L2.35253 14.0282C2.08218 13.7641 1.86736 13.4487 1.72069 13.1003C1.57402 12.752 1.49846 12.3779 1.49846 12C1.49846 11.6221 1.57402 11.248 1.72069 10.8997C1.86736 10.5513 2.08218 10.2359 2.35253 9.9718L3.76867 8.59016L3.74467 6.61296C3.74025 6.23519 3.81137 5.86035 3.95386 5.51045C4.09634 5.16056 4.30732 4.84266 4.57439 4.57545C4.84146 4.30824 5.15924 4.0971 5.50906 3.95443C5.85888 3.81176 6.23369 3.74045 6.61146 3.74467L8.59166 3.76867L9.9718 2.35253C10.2359 2.08218 10.5513 1.86736 10.8997 1.72069C11.248 1.57402 11.6221 1.49846 12 1.49846C12.3779 1.49846 12.752 1.57402 13.1003 1.72069C13.4487 1.86736 13.7641 2.08218 14.0282 2.35253ZM16.0339 2.26102L15.1008 1.30542C14.697 0.892193 14.2147 0.563841 13.6822 0.339662C13.1497 0.115483 12.5778 0 12 0C11.4222 0 10.8503 0.115483 10.3178 0.339662C9.7853 0.563841 9.30298 0.892193 8.89919 1.30542L7.96609 2.25952L6.63096 2.24452C6.05303 2.23768 5.47956 2.34647 4.94428 2.56448C4.40901 2.7825 3.92272 3.10535 3.51404 3.51404C3.10535 3.92272 2.7825 4.40901 2.56448 4.94428C2.34647 5.47956 2.23768 6.05303 2.24452 6.63096L2.26102 7.96609L1.30542 8.89919C0.892193 9.30298 0.563841 9.7853 0.339662 10.3178C0.115483 10.8503 0 11.4222 0 12C0 12.5778 0.115483 13.1497 0.339662 13.6822C0.563841 14.2147 0.892193 14.697 1.30542 15.1008L2.25952 16.0339L2.24452 17.369C2.23768 17.947 2.34647 18.5204 2.56448 19.0557C2.7825 19.591 3.10535 20.0773 3.51404 20.486C3.92272 20.8947 4.40901 21.2175 4.94428 21.4355C5.47956 21.6535 6.05303 21.7623 6.63096 21.7555L7.96609 21.739L8.89919 22.6946C9.30298 23.1078 9.7853 23.4362 10.3178 23.6603C10.8503 23.8845 11.4222 24 12 24C12.5778 24 13.1497 23.8845 13.6822 23.6603C14.2147 23.4362 14.697 23.1078 15.1008 22.6946L16.0339 21.7405L17.369 21.7555C17.947 21.7623 18.5204 21.6535 19.0557 21.4355C19.591 21.2175 20.0773 20.8947 20.486 20.486C20.8947 20.0773 21.2175 19.591 21.4355 19.0557C21.6535 18.5204 21.7623 17.947 21.7555 17.369L21.739 16.0339L22.6946 15.1008C23.1078 14.697 23.4362 14.2147 23.6603 13.6822C23.8845 13.1497 24 12.5778 24 12C24 11.4222 23.8845 10.8503 23.6603 10.3178C23.4362 9.7853 23.1078 9.30298 22.6946 8.89919L21.7405 7.96609L21.7555 6.63096C21.7623 6.05303 21.6535 5.47956 21.4355 4.94428C21.2175 4.40901 20.8947 3.92272 20.486 3.51404C20.0773 3.10535 19.591 2.7825 19.0557 2.56448C18.5204 2.34647 17.947 2.23768 17.369 2.24452L16.0339 2.26102ZM15.6945 9.46229C15.6567 9.37117 15.6013 9.2884 15.5314 9.21872C15.4617 9.14887 15.379 9.09345 15.2878 9.05563C15.1967 9.01782 15.099 8.99836 15.0003 8.99836C14.9017 8.99836 14.804 9.01782 14.7129 9.05563C14.6217 9.09345 14.539 9.14887 14.4693 9.21872L10.4999 13.1896L8.78072 11.4689C8.71098 11.3992 8.62819 11.3439 8.53707 11.3061C8.44596 11.2684 8.3483 11.249 8.24967 11.249C8.15104 11.249 8.05338 11.2684 7.96226 11.3061C7.87115 11.3439 7.78835 11.3992 7.71862 11.4689C7.64888 11.5387 7.59356 11.6215 7.55581 11.7126C7.51807 11.8037 7.49865 11.9014 7.49865 12C7.49865 12.0986 7.51807 12.1963 7.55581 12.2874C7.59356 12.3785 7.64888 12.4613 7.71862 12.5311L9.96884 14.7813C10.0385 14.8511 10.1213 14.9066 10.2124 14.9444C10.3035 14.9822 10.4012 15.0016 10.4999 15.0016C10.5986 15.0016 10.6962 14.9822 10.7874 14.9444C10.8785 14.9066 10.9613 14.8511 11.0309 14.7813L15.5314 10.2808C15.6013 10.2112 15.6567 10.1284 15.6945 10.0373C15.7323 9.94613 15.7518 9.84843 15.7518 9.74977C15.7518 9.65111 15.7323 9.55342 15.6945 9.46229Z"
                                                                fill="#D9D9D9"
                                                            />
                                                        </svg>
                                                        Active vCards
                                                    </div>
                                                </Link>
                                            </li>
                                            <li>
                                                <Link href="/">
                                                    <div>
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            width="24"
                                                            height="24"
                                                            viewBox="0 0 24 24"
                                                            fill="none">
                                                            <path
                                                                fillRule="evenodd"
                                                                clipRule="evenodd"
                                                                d="M4.57538 19.4246C6.54451 21.3938 9.21523 22.5 12 22.5C14.7848 22.5 17.4555 21.3938 19.4246 19.4246C21.3938 17.4555 22.5 14.7848 22.5 12C22.5 9.21523 21.3938 6.54451 19.4246 4.57538C17.4555 2.60625 14.7848 1.5 12 1.5C9.21523 1.5 6.54451 2.60625 4.57538 4.57538C2.60625 6.54451 1.5 9.21523 1.5 12C1.5 14.7848 2.60625 17.4555 4.57538 19.4246ZM20.4853 20.4853C18.2348 22.7357 15.1826 24 12 24C8.8174 24 5.76516 22.7357 3.51472 20.4853C1.26428 18.2348 0 15.1826 0 12C0 8.8174 1.26428 5.76516 3.51472 3.51472C5.76516 1.26428 8.8174 0 12 0C15.1826 0 18.2348 1.26428 20.4853 3.51472C22.7357 5.76516 24 8.8174 24 12C24 15.1826 22.7357 18.2348 20.4853 20.4853ZM8.04918 8.04918C7.69754 8.40081 7.5 8.87772 7.5 9.375V14.625C7.5 15.1223 7.69754 15.5992 8.04918 15.9508C8.40081 16.3025 8.87772 16.5 9.375 16.5C9.87228 16.5 10.3492 16.3025 10.7008 15.9508C11.0525 15.5992 11.25 15.1223 11.25 14.625V9.375C11.25 8.87772 11.0525 8.40081 10.7008 8.04918C10.3492 7.69754 9.87228 7.5 9.375 7.5C8.87772 7.5 8.40081 7.69754 8.04918 8.04918ZM13.2992 8.04918C12.9475 8.40081 12.75 8.87772 12.75 9.375V14.625C12.75 15.1223 12.9475 15.5992 13.2992 15.9508C13.6508 16.3025 14.1277 16.5 14.625 16.5C15.1223 16.5 15.5992 16.3025 15.9508 15.9508C16.3025 15.5992 16.5 15.1223 16.5 14.625V9.375C16.5 8.87772 16.3025 8.40081 15.9508 8.04918C15.5992 7.69754 15.1223 7.5 14.625 7.5C14.1277 7.5 13.6508 7.69754 13.2992 8.04918Z"
                                                                fill="#D9D9D9"
                                                            />
                                                        </svg>
                                                        Paused vCards
                                                    </div>
                                                </Link>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="col-md-8">
                            <div className="qr-show">
                                <div className="qr-nav">
                                    <div className="qr-count">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="36"
                                            height="36"
                                            viewBox="0 0 36 36"
                                            fill="none">
                                            <path
                                                fillRule="evenodd"
                                                clipRule="evenodd"
                                                d="M21.0423 3.5288L23.1148 5.65301L26.0806 5.61701C26.6472 5.61037 27.2095 5.71706 27.7343 5.93078C28.2592 6.14451 28.736 6.46097 29.1368 6.86158C29.5376 7.26219 29.8544 7.73886 30.0684 8.26359C30.2824 8.78832 30.3893 9.35053 30.383 9.91719L30.347 12.8875L32.4712 14.9577C32.8767 15.3538 33.199 15.827 33.419 16.3495C33.639 16.8719 33.7523 17.4331 33.7523 18C33.7523 18.5669 33.639 19.1281 33.419 19.6505C33.199 20.173 32.8767 20.6462 32.4712 21.0423L30.347 23.1148L30.383 26.0806C30.3896 26.6472 30.2829 27.2095 30.0692 27.7343C29.8555 28.2592 29.539 28.736 29.1384 29.1368C28.7378 29.5376 28.2611 29.8544 27.7364 30.0684C27.2117 30.2824 26.6495 30.3893 26.0828 30.383L23.1125 30.347L21.0423 32.4712C20.6462 32.8767 20.173 33.199 19.6505 33.419C19.1281 33.639 18.5669 33.7523 18 33.7523C17.4331 33.7523 16.8719 33.639 16.3495 33.419C15.827 33.199 15.3538 32.8767 14.9577 32.4712L12.8852 30.347L9.91944 30.383C9.35279 30.3896 8.79052 30.2829 8.26568 30.0692C7.74083 29.8555 7.264 29.539 6.86318 29.1384C6.46236 28.7378 6.14565 28.2611 5.93165 27.7364C5.71765 27.2117 5.61067 26.6495 5.61701 26.0828L5.65301 23.1125L3.5288 21.0423C3.12327 20.6462 2.80104 20.173 2.58103 19.6505C2.36103 19.1281 2.2477 18.5669 2.2477 18C2.2477 17.4331 2.36103 16.8719 2.58103 16.3495C2.80104 15.827 3.12327 15.3538 3.5288 14.9577L5.65301 12.8852L5.61701 9.91944C5.61037 9.35279 5.71706 8.79052 5.93078 8.26568C6.14451 7.74083 6.46097 7.264 6.86158 6.86318C7.26219 6.46236 7.73886 6.14565 8.26359 5.93165C8.78832 5.71765 9.35053 5.61067 9.91719 5.61701L12.8875 5.65301L14.9577 3.5288C15.3538 3.12327 15.827 2.80104 16.3495 2.58103C16.8719 2.36103 17.4331 2.2477 18 2.2477C18.5669 2.2477 19.1281 2.36103 19.6505 2.58103C20.173 2.80104 20.6462 3.12327 21.0423 3.5288ZM24.0509 3.39153L22.6512 1.95814C22.0455 1.33829 21.3221 0.845761 20.5233 0.509492C19.7246 0.173224 18.8666 0 18 0C17.1334 0 16.2755 0.173224 15.4767 0.509492C14.678 0.845761 13.9545 1.33829 13.3488 1.95814L11.9491 3.38928L9.94644 3.36678C9.07954 3.35652 8.21934 3.5197 7.41643 3.84673C6.61351 4.17375 5.88409 4.65802 5.27105 5.27105C4.65802 5.88409 4.17375 6.61351 3.84673 7.41643C3.5197 8.21934 3.35652 9.07954 3.36678 9.94644L3.39153 11.9491L1.95814 13.3488C1.33829 13.9545 0.845761 14.678 0.509492 15.4767C0.173224 16.2755 0 17.1334 0 18C0 18.8666 0.173224 19.7246 0.509492 20.5233C0.845761 21.3221 1.33829 22.0455 1.95814 22.6512L3.38928 24.0509L3.36678 26.0536C3.35652 26.9205 3.5197 27.7807 3.84673 28.5836C4.17375 29.3865 4.65802 30.1159 5.27105 30.729C5.88409 31.342 6.61351 31.8263 7.41643 32.1533C8.21934 32.4803 9.07954 32.6435 9.94644 32.6332L11.9491 32.6085L13.3488 34.0419C13.9545 34.6617 14.678 35.1542 15.4767 35.4905C16.2755 35.8268 17.1334 36 18 36C18.8666 36 19.7246 35.8268 20.5233 35.4905C21.3221 35.1542 22.0455 34.6617 22.6512 34.0419L24.0509 32.6107L26.0536 32.6332C26.9205 32.6435 27.7807 32.4803 28.5836 32.1533C29.3865 31.8263 30.1159 31.342 30.729 30.729C31.342 30.1159 31.8263 29.3865 32.1533 28.5836C32.4803 27.7807 32.6435 26.9205 32.6332 26.0536L32.6085 24.0509L34.0419 22.6512C34.6617 22.0455 35.1542 21.3221 35.4905 20.5233C35.8268 19.7246 36 18.8666 36 18C36 17.1334 35.8268 16.2755 35.4905 15.4767C35.1542 14.678 34.6617 13.9545 34.0419 13.3488L32.6107 11.9491L32.6332 9.94644C32.6435 9.07954 32.4803 8.21934 32.1533 7.41643C31.8263 6.61351 31.342 5.88409 30.729 5.27105C30.1159 4.65802 29.3865 4.17375 28.5836 3.84673C27.7807 3.5197 26.9205 3.35652 26.0536 3.36678L24.0509 3.39153ZM23.5416 14.1934C23.4848 14.0567 23.4017 13.9326 23.2969 13.8281C23.1924 13.7233 23.0683 13.6402 22.9316 13.5834C22.7949 13.5267 22.6483 13.4975 22.5004 13.4975C22.3524 13.4975 22.2058 13.5267 22.0691 13.5834C21.9324 13.6402 21.8083 13.7233 21.7038 13.8281L15.7497 19.7844L13.1709 17.2034C13.0663 17.0988 12.9421 17.0158 12.8054 16.9592C12.6688 16.9026 12.5223 16.8735 12.3743 16.8735C12.2264 16.8735 12.0799 16.9026 11.9432 16.9592C11.8066 17.0158 11.6824 17.0988 11.5778 17.2034C11.4731 17.308 11.3902 17.4322 11.3336 17.5689C11.2769 17.7056 11.2478 17.8521 11.2478 18C11.2478 18.1479 11.2769 18.2944 11.3336 18.4311C11.3902 18.5678 11.4731 18.692 11.5778 18.7966L14.9531 22.1719C15.0576 22.2767 15.1818 22.3598 15.3185 22.4165C15.4551 22.4733 15.6017 22.5025 15.7497 22.5025C15.8977 22.5025 16.0442 22.4733 16.1809 22.4165C16.3176 22.3598 16.4417 22.2767 16.5463 22.1719L23.2969 15.4212C23.4017 15.3167 23.4848 15.1926 23.5416 15.0559C23.5983 14.9192 23.6275 14.7726 23.6275 14.6247C23.6275 14.4767 23.5983 14.3301 23.5416 14.1934Z"
                                                fill="#00B2D9"
                                            />
                                        </svg>
                                        <p>
                                            Active vCards <span>(03)</span>
                                        </p>
                                    </div>

                                    <div className="qr-button">
                                        <Link href="/createqr">
                                            <button className="submit-form">
                                                Create New vCard
                                            </button>
                                        </Link>
                                    </div>
                                </div>
                            </div>

                            <div className="qr-list">
                                <ul className="qe-ul">
                                    {getqr.map(qr => (
                                        <li key={qr.id}>
                                            <div className="row">
                                                <div className="col-md-6">
                                                    <div className="compnay-info">
                                                        <div className="qr-logo">
                                                            <img
                                                                src="img/logo-qr.png"
                                                                alt=""
                                                            />
                                                        </div>
                                                        <div className="compnay-name-info">
                                                            <h3>
                                                                {qr.cardname}
                                                            </h3>

                                                            <p>
                                                                <svg
                                                                    xmlns="http://www.w3.org/2000/svg"
                                                                    width="16"
                                                                    height="16"
                                                                    viewBox="0 0 16 16"
                                                                    fill="none">
                                                                    <path
                                                                        fillRule="evenodd"
                                                                        clipRule="evenodd"
                                                                        d="M4.89104 5.66436C5.10112 5.00339 5.46669 4.40247 5.95706 3.91202L8.59904 1.27005C9.41223 0.45685 10.5152 0 11.6652 0C12.8152 0 13.9182 0.45685 14.7314 1.27005C15.5446 2.08324 16.0014 3.18618 16.0014 4.33621C16.0014 5.48625 15.5446 6.58918 14.7314 7.40238L12.7484 9.3853C12.7875 8.76094 12.7253 8.13224 12.5634 7.52522L13.7096 6.38056C14.2519 5.83817 14.5566 5.10254 14.5566 4.33549C14.5566 3.56844 14.2519 2.8328 13.7096 2.29042C13.1672 1.74803 12.4315 1.44332 11.6645 1.44332C10.8974 1.44332 10.1618 1.74803 9.61941 2.29042L6.97888 4.93239C6.62011 5.29112 6.36208 5.73796 6.23072 6.22801C6.09936 6.71806 6.09929 7.23405 6.23053 7.72413C6.36176 8.21421 6.61967 8.66111 6.97835 9.01994C7.33702 9.37876 7.78382 9.63686 8.27385 9.7683L7.15376 10.8898C6.52798 10.5908 5.98308 10.146 5.56477 9.59284C5.14646 9.03964 4.86696 8.39417 4.74973 7.7106C4.6325 7.02704 4.68096 6.32532 4.89104 5.66436ZM1.27005 8.59763L3.25297 6.6147C3.21379 7.24065 3.27626 7.86736 3.43797 8.47333L2.29186 9.618C1.74947 10.1604 1.44476 10.896 1.44476 11.6631C1.44476 12.4301 1.74947 13.1658 2.29186 13.7081C2.83425 14.2505 3.56988 14.5552 4.33693 14.5552C5.10399 14.5552 5.83962 14.2505 6.38201 13.7081L9.02254 11.0662C9.37842 10.7106 9.63535 10.2683 9.76791 9.78303C9.90048 9.29775 9.9041 8.78626 9.77842 8.29915C9.65274 7.81203 9.4021 7.36616 9.05129 7.00559C8.70048 6.64502 8.26163 6.38226 7.77815 6.24326C7.83996 6.13842 7.91474 6.04179 8.00072 5.95565L8.84766 5.10872C9.47343 5.40774 10.0183 5.85252 10.4366 6.40572C10.855 6.95892 11.1344 7.60439 11.2517 8.28795C11.3689 8.97152 11.3205 9.67324 11.1104 10.3342C10.9003 10.9952 10.5347 11.5961 10.0443 12.0865L7.40238 14.73C6.58918 15.5432 5.48625 16 4.33621 16C3.18618 16 2.08324 15.5432 1.27005 14.73C0.456849 13.9168 0 12.8138 0 11.6638C0 10.5138 0.456849 9.41082 1.27005 8.59763Z"
                                                                        fill="#B5B5B5"
                                                                    />
                                                                </svg>
                                                                <span>
                                                                    {`smartcardgenerator.net/${qr.slug.substring(
                                                                        0,
                                                                        5,
                                                                    )}`}
                                                                </span>
                                                            </p>

                                                            <p>
                                                                <svg
                                                                    xmlns="http://www.w3.org/2000/svg"
                                                                    width="16"
                                                                    height="16"
                                                                    viewBox="0 0 16 16"
                                                                    fill="none">
                                                                    <path
                                                                        fillRule="evenodd"
                                                                        clipRule="evenodd"
                                                                        d="M13.6569 13.6569C12.1566 15.1571 10.1217 16 8 16C5.87827 16 3.84344 15.1571 2.34315 13.6569C0.842855 12.1566 0 10.1217 0 8C0 5.87827 0.842855 3.84344 2.34315 2.34315C3.84344 0.842855 5.87827 0 8 0C10.1217 0 12.1566 0.842855 13.6569 2.34315C15.1571 3.84344 16 5.87827 16 8C16 10.1217 15.1571 12.1566 13.6569 13.6569ZM12.9497 12.9497C14.2625 11.637 15 9.85652 15 8C15 6.14348 14.2625 4.36301 12.9497 3.05025C11.637 1.7375 9.85652 1 8 1C6.14348 1 4.36301 1.7375 3.05025 3.05025C1.7375 4.36301 1 6.14348 1 8C1 9.85652 1.7375 11.637 3.05025 12.9497C4.36301 14.2625 6.14348 15 8 15C9.85652 15 11.637 14.2625 12.9497 12.9497ZM7.85355 3.14645C7.94732 3.24021 8 3.36739 8 3.5V8.71L11.248 10.566C11.3598 10.6334 11.4408 10.7419 11.4736 10.8683C11.5065 10.9946 11.4886 11.1288 11.4238 11.2422C11.3591 11.3556 11.2525 11.4391 11.127 11.4749C11.0014 11.5108 10.8669 11.4961 10.752 11.434L7.252 9.434C7.17547 9.39029 7.11186 9.32712 7.06761 9.25091C7.02335 9.17469 7.00003 9.08813 7 9V3.5C7 3.36739 7.05268 3.24021 7.14645 3.14645C7.24021 3.05268 7.36739 3 7.5 3C7.63261 3 7.75979 3.05268 7.85355 3.14645Z"
                                                                        fill="#B5B5B5"
                                                                    />
                                                                </svg>
                                                                <span>
                                                                    Oct 2, 2023
                                                                </span>
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="col-md-6">
                                                    <div className="qr-details">
                                                        <div className="scan-details">
                                                            <div className="scan">
                                                                <p>3</p>
                                                                <p>scans</p>
                                                            </div>
                                                            <div className="details">
                                                                <Link href="details">
                                                                    Details
                                                                </Link>
                                                            </div>
                                                        </div>

                                                        <div className="scan-ain">
                                                            <div
                                                                className="margin:auto"
                                                                style={{
                                                                    display:
                                                                        'flex',
                                                                    alignItems:
                                                                        'center',
                                                                    justifyContent:
                                                                        'center',
                                                                }}>
                                                                <div
                                                                    ref={
                                                                        componentRef
                                                                    }
                                                                    style={{
                                                                        position:
                                                                            'relative',
                                                                        width:
                                                                            '100px',
                                                                        height:
                                                                            '100px',
                                                                    }}>
                                                                    <QRCode
                                                                        value={`https://smartcardgenerator.net/${qr.slug}`}
                                                                        size={
                                                                            100
                                                                        }
                                                                    />
                                                                    <img
                                                                        src={`${baseuri}/${qr.welcome}`}
                                                                        width={
                                                                            30
                                                                        }
                                                                        height={
                                                                            30
                                                                        }
                                                                        style={{
                                                                            position:
                                                                                'absolute',
                                                                            top:
                                                                                '50%',
                                                                            left:
                                                                                '50%',
                                                                            transform:
                                                                                'translate(-50%, -50%)',
                                                                        }}
                                                                    />
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div className="download-action">
                                                            <a
                                                                onClick={
                                                                    handlePrint
                                                                }
                                                                className="qr-downlod">
                                                                Download
                                                            </a>

                                                            <div className="qr-action">
                                                                <Link
                                                                    href={`/update/[id]`}
                                                                    as={`/update/${qr.id}`}>
                                                                    <svg
                                                                        xmlns="http://www.w3.org/2000/svg"
                                                                        width="16"
                                                                        height="16"
                                                                        viewBox="0 0 16 16"
                                                                        fill="none">
                                                                        <path
                                                                            d="M12.8536 0.146391C12.7599 0.052657 12.6327 0 12.5001 0C12.3676 0 12.2404 0.052657 12.1467 0.146391L10.4997 1.79337L14.2066 5.50032L15.8536 3.85435C15.9002 3.8079 15.9371 3.75273 15.9623 3.69198C15.9875 3.63124 16.0005 3.56612 16.0005 3.50035C16.0005 3.43458 15.9875 3.36946 15.9623 3.30872C15.9371 3.24797 15.9002 3.1928 15.8536 3.14635L12.8536 0.146391ZM13.4996 6.20732L9.79268 2.50036L3.29276 9.00028H3.49976C3.63237 9.00028 3.75954 9.05296 3.85331 9.14672C3.94708 9.24049 3.99976 9.36767 3.99976 9.50027V10.0003H4.49975C4.63236 10.0003 4.75953 10.0529 4.8533 10.1467C4.94707 10.2405 4.99974 10.3677 4.99974 10.5003V11.0003H5.49974C5.63234 11.0003 5.75952 11.0529 5.85329 11.1467C5.94705 11.2405 5.99973 11.3676 5.99973 11.5002V12.0002H6.49972C6.63233 12.0002 6.75951 12.0529 6.85327 12.1467C6.94704 12.2405 6.99972 12.3676 6.99972 12.5002V12.7072L13.4996 6.20732ZM6.03173 13.6752C6.01069 13.6193 5.99985 13.56 5.99973 13.5002V13.0002H5.49974C5.36713 13.0002 5.23996 12.9476 5.14619 12.8538C5.05242 12.76 4.99974 12.6328 4.99974 12.5002V12.0002H4.49975C4.36714 12.0002 4.23997 11.9476 4.1462 11.8538C4.05243 11.76 3.99976 11.6329 3.99976 11.5002V11.0003H3.49976C3.36716 11.0003 3.23998 10.9476 3.14621 10.8538C3.05245 10.76 2.99977 10.6329 2.99977 10.5003V10.0003H2.49977C2.44 10.0002 2.38072 9.98933 2.32478 9.96827L2.14578 10.1463C2.09813 10.1943 2.06071 10.2514 2.03578 10.3143L0.0358056 15.3142C-0.000564587 15.4051 -0.00946798 15.5046 0.0101992 15.6005C0.0298664 15.6964 0.0772393 15.7843 0.146445 15.8536C0.215651 15.9228 0.303645 15.9701 0.399521 15.9898C0.495396 16.0095 0.594935 16.0006 0.685798 15.9642L5.68573 13.9642C5.7486 13.9393 5.80575 13.9019 5.85373 13.8542L6.03173 13.6752Z"
                                                                            fill="#B5B5B5"
                                                                        />
                                                                    </svg>
                                                                </Link>
                                                                <Link href="/">
                                                                    <svg
                                                                        xmlns="http://www.w3.org/2000/svg"
                                                                        width="16"
                                                                        height="16"
                                                                        viewBox="0 0 16 16"
                                                                        fill="none">
                                                                        <path
                                                                            fillRule="evenodd"
                                                                            clipRule="evenodd"
                                                                            d="M3.05025 12.9497C4.36301 14.2625 6.14348 15 8 15C9.85652 15 11.637 14.2625 12.9497 12.9497C14.2625 11.637 15 9.85652 15 8C15 6.14348 14.2625 4.36301 12.9497 3.05025C11.637 1.7375 9.85652 1 8 1C6.14348 1 4.36301 1.7375 3.05025 3.05025C1.7375 4.36301 1 6.14348 1 8C1 9.85652 1.7375 11.637 3.05025 12.9497ZM13.6569 13.6569C12.1566 15.1571 10.1217 16 8 16C5.87827 16 3.84344 15.1571 2.34315 13.6569C0.842855 12.1566 0 10.1217 0 8C0 5.87827 0.842855 3.84344 2.34315 2.34315C3.84344 0.842855 5.87827 0 8 0C10.1217 0 12.1566 0.842855 13.6569 2.34315C15.1571 3.84344 16 5.87827 16 8C16 10.1217 15.1571 12.1566 13.6569 13.6569ZM5.36612 5.36612C5.1317 5.60054 5 5.91848 5 6.25V9.75C5 10.0815 5.1317 10.3995 5.36612 10.6339C5.60054 10.8683 5.91848 11 6.25 11C6.58152 11 6.89946 10.8683 7.13388 10.6339C7.3683 10.3995 7.5 10.0815 7.5 9.75V6.25C7.5 5.91848 7.3683 5.60054 7.13388 5.36612C6.89946 5.1317 6.58152 5 6.25 5C5.91848 5 5.60054 5.1317 5.36612 5.36612ZM8.86612 5.36612C8.6317 5.60054 8.5 5.91848 8.5 6.25V9.75C8.5 10.0815 8.6317 10.3995 8.86612 10.6339C9.10054 10.8683 9.41848 11 9.75 11C10.0815 11 10.3995 10.8683 10.6339 10.6339C10.8683 10.3995 11 10.0815 11 9.75V6.25C11 5.91848 10.8683 5.60054 10.6339 5.36612C10.3995 5.1317 10.0815 5 9.75 5C9.41848 5 9.10054 5.1317 8.86612 5.36612Z"
                                                                            fill="#B5B5B5"
                                                                        />
                                                                    </svg>
                                                                </Link>
                                                                <button
                                                                    onClick={() =>
                                                                        handleDelete(
                                                                            qr.id,
                                                                        )
                                                                    }>
                                                                    <svg
                                                                        xmlns="http://www.w3.org/2000/svg"
                                                                        width="16"
                                                                        height="12"
                                                                        viewBox="0 0 16 12"
                                                                        fill="none">
                                                                        <path
                                                                            fillRule="evenodd"
                                                                            clipRule="evenodd"
                                                                            d="M8.121 5.12112C7.55841 5.68371 6.79538 5.99976 5.99976 5.99976C5.20415 5.99976 4.44111 5.68371 3.87853 5.12112C3.31594 4.55853 2.99988 3.7955 2.99988 2.99988C2.99988 2.20426 3.31594 1.44123 3.87853 0.878645C4.44111 0.316058 5.20415 0 5.99976 0C6.79538 0 7.55841 0.316058 8.121 0.878645C8.68359 1.44123 8.99965 2.20426 8.99965 2.99988C8.99965 3.7955 8.68359 4.55853 8.121 5.12112ZM7.41392 4.41404C7.78898 4.03898 7.99968 3.53029 7.99968 2.99988C7.99968 2.46947 7.78898 1.96078 7.41392 1.58572C7.03886 1.21067 6.53018 0.999961 5.99976 0.999961C5.46935 0.999961 4.96066 1.21067 4.58561 1.58572C4.21055 1.96078 3.99984 2.46947 3.99984 2.99988C3.99984 3.53029 4.21055 4.03898 4.58561 4.41404C4.96066 4.7891 5.46935 4.9998 5.99976 4.9998C6.53018 4.9998 7.03886 4.7891 7.41392 4.41404ZM10.9996 11.9995C10.9996 11.9995 11.9995 11.9995 11.9995 10.9996C11.9995 9.99961 10.9996 6.99972 5.99976 6.99972C0.999961 6.99972 0 9.99961 0 10.9996C0 11.9995 0.999961 11.9995 0.999961 11.9995H10.9996ZM10.1676 9.33163C10.8456 10.0096 10.9986 10.7496 10.9996 10.9956H0.999961C1.00196 10.7496 1.15395 10.0096 1.83193 9.33163C2.4839 8.67966 3.70985 7.99968 5.99976 7.99968C8.28867 7.99968 9.51562 8.67966 10.1676 9.33163ZM12.3079 3.03717C12.2472 3.06237 12.192 3.09931 12.1456 3.14588C12.099 3.19232 12.0621 3.24749 12.0368 3.30824C12.0116 3.36898 11.9987 3.4341 11.9987 3.49986C11.9987 3.56563 12.0116 3.63075 12.0368 3.69149C12.0621 3.75223 12.099 3.8074 12.1456 3.85385L13.2925 4.9998L12.1456 6.14576C12.0517 6.23964 11.9989 6.36697 11.9989 6.49974C11.9989 6.63251 12.0517 6.75985 12.1456 6.85373C12.2394 6.94761 12.3668 7.00036 12.4995 7.00036C12.6323 7.00036 12.7596 6.94761 12.8535 6.85373L13.9995 5.70677L15.1454 6.85373C15.2393 6.94761 15.3667 7.00036 15.4994 7.00036C15.6322 7.00036 15.7595 6.94761 15.8534 6.85373C15.9473 6.75985 16 6.63251 16 6.49974C16 6.36697 15.9473 6.23964 15.8534 6.14576L14.7065 4.9998L15.8534 3.85385C15.9473 3.75997 16 3.63263 16 3.49986C16 3.36709 15.9473 3.23976 15.8534 3.14588C15.7595 3.05199 15.6322 2.99925 15.4994 2.99925C15.3667 2.99925 15.2393 3.05199 15.1454 3.14588L13.9995 4.29283L12.8535 3.14588C12.8071 3.09931 12.7519 3.06237 12.6912 3.03717C12.6304 3.01196 12.5653 2.99899 12.4995 2.99899C12.4338 2.99899 12.3687 3.01196 12.3079 3.03717Z"
                                                                            fill="#B5B5B5"
                                                                        />
                                                                    </svg>
                                                                </button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </AppLayout>
    )
}

export default Dashboard
